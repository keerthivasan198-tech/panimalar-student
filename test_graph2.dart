...
class CampusPathGraph {
  ...
  CampusPathGraph() {
    ...
    for (final e in _edges) {
      if (e[0] == e[1]) continue;
      if (!_adj.containsKey(e[0]) || !_adj.containsKey(e[1])) continue;
      if (!_adj[e[0]]!.contains(e[1])) _adj[e[0]]!.add(e[1]);
      if (!_adj[e[1]]!.contains(e[0])) _adj[e[1]]!.add(e[0]);
    }
  }
  double _dist(LatLng a, LatLng b) {
    ...
    final x = sin(dp / 2) * sin(dp / 2) + cos(p1) * cos(p2) * sin(dl / 2) * sin(dl / 2);
  ...
  String _snapBuilding(String name, LatLng fallback) => _buildingSnap[name] ?? _snap(fallback);
  List<LatLng> shortestPath(LatLng origin, LatLng destination, {String? destName}) {
    final startId = _snap(origin);
    final endId = destName != null ? _snapBuilding(destName, destination) : _snap(destination);
    print('Start: ' + startId + ', End: ' + endId);
    if (startId == endId) return [origin, _nodeMap[startId]!.pos, destination];
    final dist = <String, double>{for (final n in _nodes) n.id: double.infinity};
    ...
    for (final id in ordered) { path.add(_nodeMap[id]!.pos); }
...
void main() {
  ...
  final path = g.shortestPath(LatLng(13.05172, 80.07600), LatLng(13.05085, 80.07474), destName: 'Mess 1 (Main Mess)');
  print('Nodes in path: ' + path.length.toString());
}
